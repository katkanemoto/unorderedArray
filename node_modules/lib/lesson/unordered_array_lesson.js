...
function lesson(options) {
	...
	*/
	...
	arrow._index
	*/
	this._tail_arrow = result._.arrow;
...
lesson.prototype = {
	...
	pre_start : function() {
		...
		this._UI.input.self._.htmlElement.disabled = true;
	...
	next : function() {
		...
		if (this._array_full) {
			...
			if (this._step_state === this._STEP_1) {
				...
				this._array_tmp = result;
				
				//store the new tail arrow for the new array
				this._arrow_tmp = result._.arrow;
			...
			if (this._step_state === this._STEP_2) {
				...
				this._array.addChild(this._move_pointer);	//add it to the array so it's visible on the stage
			...
			if (this._step_state === this._STEP_MOVING_1) {
				...
				this._array_tmp._.push({value:this._move_pointer._.pointed_at.children[1]});
			...
			if (this._step_state === this._STEP_1) {
				...
				this._array._.push({value:input});
			...
			if (this._step_state === this._STEP_2) {
				...
				//re-enable the text input for the next lesson
				this._UI.input.self._.htmlElement.disabled = false;
