...
function lesson(options) {
	...
	this._UI.next_button.addEventListener("click", function() {
		//DEBUG
		//console.log("[next button] saw click event");
		//DEBUG
...
lesson.prototype = {
	...
	pre_start : function() {
		if (this._state !== this._READY) {
			var message = "Press the 'Next' button" + "\n" +
	...
	next : function() {
		...
		if (this._array_full) {
			...
			if (this._step_state === this._STEP_MOVING_4) {
				...
				if (this._move_pointer._.index === this._UI.arrow.working._.index) {
			...
			if (this._step_state === this._STEP_3) {
				...
				var array_x = this._UI.array.working.x;
				var array_y = this._UI.array.working.y;
				
				//remove the old array (and arrow) from the UI
				this._UI.stage.del(this._UI.array.working);
				...
				this._UI.array.working._.move.to({x:array_x, y:array_y});
