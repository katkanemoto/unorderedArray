...
test('[lesson.next] correct, search complete (found)', function (t) {
	//SETUP
	var ui = SETUP();
	var l = lesson({UI: ui});
	ui.input.set("1");
	...
	var test_array = array_setup({size:4, fill:[1, 2, 3, 4]});
	...
	var intervalID = setInterval(function() {
		...
		if (l._step_state === l._STEP_INIT_DELETE) {
			t.equal(result.value.index, 0);
			t.ok(result.value.found);
			
			t.ok(check.object(l._delete));
...
test('[lesson.next] correct, search complete (not found)', function (t) {
	//SETUP
	var ui = SETUP();
	var l = lesson({UI: ui});
	ui.input.set("0");
	...
	var intervalID = setInterval(function() {
		...
		if (l._state === l._READY) {
			t.ok(result.value.match(/not locate/));
			t.ok(result.value.match(/delete/));
			
			t.equal(l._state, l._READY);
			t.equal(l._step_state, undefined);
...
test('[lesson.next] correct, _STEP_INIT_DELETE', function (t) {
	//SETUP
	var ui = SETUP();
	var l = lesson({UI: ui});
	ui.input.set("0");
	...
	l._state = l._RUN;
	l._step_state = l._STEP_INIT_DELETE;
	...
	t.ok(result.value.match(/delete/));
	t.ok(result.value.match(/shrink/));
	
	t.equal(l._STEP_DELETING, l._step_state);
	
	TEARDOWN();
	t.end();
});

test('[lesson.next] correct, complete delete, beginning of array', function (t) {
	//SETUP
	var ui = SETUP();
	var l = lesson({UI: ui});
	ui.input.set("0");
	...
	var array_setup = require('lib/lesson/util/array_setup');
	var test_array = array_setup({size:6, fill:[0, 1, 2, 3, 4, 5], x:50, y:145});
	var test_arrow = test_array._.arrow;
	...
	//iterate through the lesson logic, until we reach the end of the lesson
	var intervalID = setInterval(function() {
		var result = l.next();
		
		if (result.done) {
			t.equal(l._array._.values.length, 6);
			t.deepEqual(l._array._.values, [1, 2, 3, 4, 5, null]);
...
test('[lesson.next] correct, complete delete, end of array', function (t) {
	//SETUP
	var ui = SETUP();
	var l = lesson({UI: ui});
	ui.input.set("5");
	...
	var intervalID = setInterval(function() {
		...
		if (result.done) {
			t.equal(l._array._.values.length, 6);
			t.deepEqual(l._array._.values, [0, 1, 2, 3, 4, null]);
...
test('[lesson.next] correct, complete delete, middle of the array', function (t) {
	//SETUP
	var ui = SETUP();
	var l = lesson({UI: ui});
	ui.input.set("3");
	...
	var intervalID = setInterval(function() {
		...
		if (result.done) {
			t.equal(l._array._.values.length, 6);
			t.deepEqual(l._array._.values, [0, 1, 2, 4, 5, null]);
...
test('[lesson.next] correct, complete delete, middle of the array', function (t) {
	...
	var array_setup = require('lib/lesson/util/array_setup');
	var test_array = array_setup({size:1, fill:[0], x:50, y:145});
	var test_arrow = test_array._.arrow;
	...
	var intervalID = setInterval(function() {
		...
		if (result.done) {
			t.equal(l._array._.values.length, 1);
			t.deepEqual(l._array._.values, [null]);
