...
test('[lesson] correct, insert button, event handler (mock)', function (t) {
	t.plan(1);
...
test('[lesson] correct, insert button event handler (mock), does nothing when tweening', function (t) {
	t.plan(5);
	...
	setTimeout(function() {
		...
		ui.insert_button.dispatchEvent(new Event("click"));	//should do nothing
	}, 500);
...
test('[lesson] correct, insert button, event handler (real)', function (t) {
	t.plan(1);
...
test('[lesson] correct, next button, event handler (mock)', function (t) {
	t.plan(1);
...
test('[lesson] correct, next button event handler (mock), does nothing when tweening', function (t) {
	t.plan(5);
...
test('[lesson] correct, next button, event handler (real)', function (t) {
	t.plan(1);
...
test('[lesson.pre_start] correct, correct state & bad input (mock)', function (t) {
	t.plan(3);
	...
	t.ok(output.match(/the insert button/), '...and what to do');
});
...
test('[lesson.pre_start] correct, working array full case detected (mock)', function (t) {
	//SETUP
	var ui = SETUP().mock();
	
	ui.array.working._.length = 1;
	ui.array.working._.values.push(-1);
	ui.array.working.addChild(new createjs.Shape());
...
test('[lesson.pre_start] correct, working array full case detected (real)', function (t) {
	...
	ui.array.working._.values = [0, 0];	//create a full array to detect
...
test('[lesson.pre_start] correct, working array NOT full case detected (mock)', function (t) {
	//SETUP
	var ui = SETUP().mock();
	
	ui.array.working._.length = 2;
	ui.array.working._.values.push(-1);
	ui.array.working.addChild(new createjs.Shape());
...
test('[lesson.pre_start] correct, working array NOT full case detected (real)', function (t) {
	...
	ui.array.working._.values = [];	//create an array with room to insert
	//create an input to attempt to insert
	ui.input.set("0");
...
test('[lesson.pre_start] correct, case: array not full, call 1 (mock)', function (t) {
	t.plan(9);
	...
	ui.array.working._.push = function() {
		t.ok(true, 'called array._.push()');
	}
	
	//create a mock set() function to check for call
	ui.output.set = function() {
		t.ok(true, 'called output.set()');
	};
	
	ui.input.get_canvas_element = function() {
		t.ok(true, 'called input.get_canvas_element()');
	};
...
test('[lesson.pre_start] correct, case: array not full, call 1 (real)', function (t) {
	...
	//PRE-TEST
	t.equal(ui.array.working._.values.length, 0, "values array is empty");
	t.equal(ui.array.working.children[0].children.length, 1, "array's first child has one child");
	...
	t.equal(ui.array.working._.values.length, 1, "values array now has a single element");
	t.equal(ui.array.working.children[0].children.length, 2, "array's first child now has two children");
...
test('[lesson.next] correct, case: array not full, call 2 (mock)', function (t) {
	t.plan(7);
	...
	ui.array.working._.values.push(0);
	//two items in the first child of the UI array
	ui.array.working.addChild(new createjs.Container());
	ui.array.working.children[0].addChild(new createjs.Shape());
	ui.array.working.children[0].addChild(new createjs.Shape());
	//input is blank
	ui.input.clear()
	
	//create mock functions to call
	ui.output.set = function() {
		t.ok(true, 'called set()');
	};
	
	ui.arrow.working._.next = function() {
		t.ok(true, 'incremented the tail');
	};
	
	//setup mock index for arrow to point at
	ui.arrow.working._.index = 1;
...
test('[lesson.next] correct, case: array not full, call 2 (real)', function (t) {
	...
	t.ok(result.value.match(/Moved the \'tail\' pointer/));
	t.ok(result.value.match(/to prepare for the next insertion/));
	t.equal(ui.arrow.working._.index, 1, 'incremented the index for the arrow');
...
test('[lesson.pre_start] correct, case: array full, call 1 (real)', function (t) {
	...
	//INIT
	//fill the array
	ui.array.working._.values = [0, 1];
	ui.array.working.children[0].addChild(new createjs.Text("0"));
	ui.array.working.children[1].addChild(new createjs.Text("1"));
	
	//point the tail arrow (old array) at index 1
	ui.arrow.working._.pointed_at = ui.array.working.children[1];
	ui.arrow.working.x += 50;
	...
	t.equal(l._array_tmp.x, ui.array.working.x, 'positioned the new array below the old one');
	t.ok(l._array_tmp.y > ui.array.working.y, 'positioned the new array below the old one');
...
test('[lesson.next] correct, case: array full, call 2 (real)', function (t) {
	...
	l._array_tmp = result.array;
	l._arrow_tmp = result.arrow;
	
	l._array_tmp.x = ui.array.working.x;
	l._array_tmp.y = ui.array.working.y + 115;
...
test('[lesson.next] correct, case: array full, call 3 - moving 1 (real)', function (t) {
	...
	ui.arrow.working._.pointed_at = undefined;
	ui.arrow.working.x += 50;
	ui.arrow.working.x += 50;
	ui.arrow.working._.index = 2;
	//END INIT
	...
	//CALL 2
	l._move_pointer = ui.array.working._.iterator("move");
	...
	t.equal(ui.array.working.children[0].children.length, 2, "first index in the old array has two children");
	t.equal(ui.array.working.children[0].children[1].text, "0", "the first value in the old array is 0, before the move");
	...
	t.equal(ui.array.working.children[0].children.length, 1, "first index in the old array now has only 1 child");
...
test('[lesson.next] correct, case: array full, call 4 - moving 2 (real)', function (t) {
	...
	//CALL 3
	ui.array.working.children[0].removeChildAt(1);	//remove the 0 from the old array
...
test('[lesson.next] correct, case: array full, call 5 - moving 3 (real)', function (t) {
	...
	l._move_pointer._.pointed_at = ui.array.working.children[1];
...
test('[lesson.next] correct, case: array full, call 6 - moving 4 (real), exiting from move', function (t) {
	...
	//CALL 3
	ui.array.working.children[0].removeChildAt(1);	//remove the 0 from the old array
	ui.array.working.children[1].removeChildAt(1);	//remove the 1 from the old array
	l._array_tmp.children[0].addChild(new createjs.Text("0"));	//and add 0 to the new array
...
test('[lesson.next] correct, case: array full, call 7 - step 3 (real)', function (t) {
	...
	//TEST
	t.deepEqual(ui.array.working, new_array, "post next() the new array should be the default working array, as the old one had been removed");
...
test('[lesson] correct, integration testing, insert into array to fill it (size 2), then insert another value to create new array (size 4) and insert into it', function (t) {
	...
	function call_3() {
		...
		//TEST
		t.ok(Number.isNaN(ui.input.get()), "input is empty");
		t.equal(ui.array.working._.values.length, 2, "now two elements in the array");
	...
	function call_4() {
		...
		t.equal(ui.arrow.working._.index, 2, 'incremented the index for the arrow');
	};
	...
	function call_8() {
		...
		t.equal(l._move_pointer._.index, 1, "move pointer should now be at index 1");
		t.deepEqual(l._move_pointer._.pointed_at, ui.array.working.children[1], "move pointer should now be at index 1");
	};
	...
	function call_11() {
		...
		t.equal(ui.array.working.children[1].children.length, 1, "target index in the old array now has only 1 child");
	};
	...
	function call_15() {
		...
		t.equal(l._UI.array.working._.length, 4, "post next() the new array should be the default working array, as the old one had been removed");
	...
	function call_16() {
		...
		t.equal(l._UI.array.working._.values.length, 3, "values array now has a single element");
	...
	function call_17() {
		...
		t.ok(result.match(/Moved the \'tail\' pointer/));
		t.ok(result.match(/to prepare for the next insertion/));
		t.equal(l._UI.arrow.working._.index, 3, 'incremented the index for the arrow');
	};
...
test('[lesson.next] correct, press Next button when not in RUN state', function (t) {
	//SETUP
	var ui = SETUP().real();
	...
	t.ok(result.match(/Enter a number above/));
	t.ok(result.match(/press 'Insert'/));
	
	t.end();
});

test('[lesson.pre_start] correct, press Insert button when not in READY state', function (t) {
	//SETUP
	var ui = SETUP().real();
	...
	t.ok(result.match(/Press the 'Next' button/));
	t.ok(result.match(/continue the lesson/));
