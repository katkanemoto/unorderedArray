...
test('[lesson.next] correct, case: array not full, call 2 (mock)', function (t) {
	...
	ui.arrow.working._.pointed_at._.index = 1;
...
test('[lesson.next] correct, case: array not full, call 2 (real)', function (t) {
	...
	t.ok(result.value.match(/Moved the \'tail\' pointer/));
	t.ok(result.value.match(/to prepare for the next insertion/));
	t.equal(ui.arrow.working._.pointed_at._.index, 1, 'incremented the index for the arrow');
...
test('[lesson.next] correct, case: array full, call 4 - moving 2 (real)', function (t) {
	...
	t.equal(l._move_pointer._.pointed_at._.index, 1, "move pointer should now be at index 1");
...
test('[lesson.next] correct, case: array full, call 5 - moving 3 (real)', function (t) {
	...
	t.equal(l._arrow_tmp._.pointed_at._.index, 1, "tail pointer in the new array, now at index 1");
	t.equal(l._step_state, l._STEP_MOVING_1, "back at the first moving sub-step, to move the next value");
...
test('[lesson.next] correct, case: array full, call 4 - exiting from move', {skip:true}, function (t) {
	t.plan(4);
...
test('[lesson.next] correct, case: array full, call 5 - removing old array and setting up new array as "working"', {skip:true}, function (t) {
	t.plan(11);
...
test('[lesson] correct, integration testing, insert into array to fill it (size 2), then insert another value to create new array (size 4) and insert into it', {skip:true}, function (t) {
	//SETUP
	var mock_UI = require('lib/test/mock_UI');
	var ui = mock_UI();
