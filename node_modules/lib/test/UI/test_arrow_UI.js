...
test("[arrow_UI] correct", function (t) {
	...
	var args = {name:"test", position:{x:0, y:0}, pointed_at:{}};
...
test('[arrow_UI._decorate], correct', function (t) {
	...
	t.ok(check.function(result._.register_next), "should have a register_next() function");
	
	TEARDOWN();
	t.end();
});

test('[arrow_UI._decorate -> register_next], incorrect, no argument', function (t) {
	t.plan(3);
	//SETUP
	SETUP();
	...
	//TEST
	try {
		result._.register_next();
	} catch(error) {
		t.ok(check.instanceStrict(error, TypeError), 'we should get back a TypeError');
		var message = error.toString();
		t.ok(message.match(/Required argument 'func'/), "tells you what's wrong");
...
test('[arrow_UI._decorate -> register_next], incorrect, bad argument type', function (t) {
	t.plan(3);
	//SETUP
	SETUP();
	...
	//TEST
	try {
		result._.register_next("some bad type");
	} catch(error) {
		t.ok(check.instanceStrict(error, TypeError), 'we should get back a TypeError');
		var message = error.toString();
		t.ok(message.match(/Required argument 'func'/), "tells you what's wrong");
...
test("[arrow_UI._decorate -> register_next] correct", function (t) {
	t.plan(1);
	//SETUP
	SETUP();
	...
	result._.register_next(function() {});
	t.ok(check.function(result._.next), "next() was registered");
	
	TEARDOWN();
});

test("[arrow_UI.is_arrow] correct", function (t) {
	//SETUP
	SETUP();
	...
	//TEST
	var result = arrow_UI(args);
	t.ok(arrow_UI.is_arrow(result));
	
	TEARDOWN();
	t.end();
});

test("[arrow_UI.is_arrow] incorrect", function (t) {
	//SETUP
	SETUP();
	
	//TEST
	t.notOk(arrow_UI.is_arrow("some wrong type"));
