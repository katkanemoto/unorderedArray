...
test('[array_UI -> pop()] incorrect, bad argument index, wrong type', function (t) {
	...
	try {
		...
		t.ok(message.match(/should be an integer within the range 0...tail/), "and how to fix it");
	}
...
test('[array_UI -> pop()] incorrect, pop() empty index in the array', function (t) {
	//SETUP
	SETUP();
	...
	try {
		...
		t.ok(check.instanceStrict(error, RangeError), 'we should get back an RangeError');
		var message = error.toString();
		t.ok(message.match(/at empty index in the array/), "tells you what's wrong");
	}
