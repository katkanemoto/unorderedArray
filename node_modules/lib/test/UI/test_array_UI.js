...
test('[array_UI._iterator] correct', function (t) {
	//SETUP
	SETUP();
	...
	//TEST
	var result = array_UI.prototype._iterator.call(mock_this, outer_container, "tail");
...
test('[array_UI._iterator] correct, next() correct from start of the array', function (t) {
	//SETUP
	SETUP();
	...
	setTimeout(function() {
		t.ok(result.x > 0 && result.x < 72, 'coordinates should be on the way to (72, 70), but not equal');
		t.equal(result.y, 70, 'y should not change');
	}, 500);
	
	setTimeout(function() {
		t.equal(result.x, 72, "arrow should be at (72, 70) after the move() is finished");
		t.equal(result.y, 70, "arrow should be at (72, 70) after the move() is finished");
...
test('[array_UI._iterator] correct, next() correct from start of the array (array positioned at (100, 100))', function (t) {
	//SETUP
	SETUP();
...
test('[array_UI._iterator] correct, next() correct to the end of the array', function (t) {
	//SETUP
	SETUP();
	...
	t.ok(check.undefined(result._.pointed_at), "arrow should be pointed past the end of the array");
	t.deepEqual(result._.index, 1, "...whose index should be 1");
...
test('[array_UI._iterator] correct, next() correct, (tween = false)', function (t) {
	//SETUP
	SETUP();
	...
	//TEST
	var result = array_UI.prototype._iterator.call(mock_this, outer_container, "tail");
	var next_result = result._.next({tween:false});
