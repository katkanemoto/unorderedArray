...
/* 
test('[lesson_UI._make_arrow -> increment], correct, unassigned', function (t) {
	t.plan(4);
	...
	t.ok(check.function(result._.increment), "'increment' funtion should be set");
	...
	try {
		...
		t.ok(check.instanceStrict(error, ReferenceError), 'we should get back a ReferenceError');
		var message = error.toString();
		t.ok(message.match(/arrow is not associated with an array/), 'tell you what was wrong');
		t.ok(message.match(/Set arrow._.array property to assign/), 'tells you what to do to fix');
	}
	
	TEARDOWN();
});

test('[lesson_UI._make_arrow -> increment], incorrect, no next element', function (t) {
	//t.plan(4);
	...
	var array = {children : [{}]};
	result._.array = array;
	result._index = 0;
	
	console.log(array.children[1]);
	//result._.increment();
