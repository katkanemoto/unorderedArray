...
test('[lesson_UI._make_array] correct', function (t) {
	...
	t.ok(setBounds_called, 'bounds on the array have been set');
});
...
test('[lesson_UI._setup_tail_arrow], correct', function (t) {
	t.plan(4);
	...
	var mock_this = {
		...
		_tail_arrow : undefined,
	};
	
	//TEST
	lesson_UI.prototype._setup_tail_arrow.call(mock_this);
	
	t.ok(called_addChild, "added the arrow to the array_screen");
	t.equal(mock_this._tail_arrow.x, -1, "placed the arrow below the first arrayElement, x");
	t.equal(mock_this._tail_arrow.y, -1, "placed the arrow below the first arrayElement, y");
	t.ok(check.object(mock_this._tail_arrow), "stored the returned arrow in _tail_arrow");
});

test('[lesson_UI._make_arrow], correct, name is set case', function (t) {
	t.plan(7);
	...
	//TEST
	var result = lesson_UI.prototype._make_arrow.call(mock_this, "test");
	...
	t.ok(check.object(result._), "should have the '_' namespace attached");
	t.ok(check.object(result._.place), "and should have place and...");
	t.ok(check.function(result._.add_to_stage), "...add_to_stage, decorated onto it");
	
	TEARDOWN();
});

test('[lesson_UI._make_arrow], correct, name is UNset case', function (t) {
	t.plan(6);
	...
	//TEST
	var result = lesson_UI.prototype._make_arrow.call(mock_this);
	
	t.ok(check.instanceStrict(result, createjs.Container), "got back a container");
	t.ok(check.instanceStrict(result.children[0], createjs.Container), "first child is the container holding the arrow");
	t.equal(result.children.length, 1, "should have just the one child, the arrow");
	t.ok(check.object(result._), "should have the '_' namespace attached");
	t.ok(check.object(result._.place), "and should have place and...");
	t.ok(check.function(result._.add_to_stage), "...add_to_stage, decorated onto it");
...
test('[lesson_UI._setup_array] correct', function (t) {
	...
	var mock_this = {
		_make_array : function(size) {
			...
			return {children:children, x:undefined, y:undefined};
		},
	};
...
test("[lesson_UI._setup_array] correct, adding positioning coordinates decorated into _.on and _.below", function (t) {
	//t.plan(3);
	...
	//TEST
	lesson_UI.prototype._setup_array.call(mock_this);
	t.ok(check.not.undefined(mock_this._array), "made an array");
	t.ok(check.not.undefined(mock_this._array.x), "should have a property x");
	t.ok(check.not.undefined(mock_this._array.y), "should have a property y");
	t.ok(mock_this._array.x !== 0, "x should be set");
	t.ok(mock_this._array.y !== 0, "y should be set");
	
	console.log(mock_this._array);
	...
	var previous_on_y = mock_this._array.children[0]._.on.y;
	
	//similar characteristics of the values of below.x and below.y
	var previous_below_x = 0;
	var previous_below_y = mock_this._array.children[0]._.below.y;
	
	for (var i = 0; i < mock_this._array.children.length; i++) {
		t.ok(check.object(mock_this._array.children[i]._), "child " + String(i) + " has a '_' namepsace");
		t.ok(check.object(mock_this._array.children[i]._.on), "child " + String(i) + " has a 'on' property");
		t.ok(check.object(mock_this._array.children[i]._.below), "child " + String(i) + " has a 'below' property");
		
		t.ok(check.number(mock_this._array.children[i]._.on.x), "child " + String(i) + " has a numeric 'on.x' property");
		t.ok(check.number(mock_this._array.children[i]._.on.y), "child " + String(i) + " has a numeric 'on.y' property");
		
		t.ok(mock_this._array.children[i]._.on.x > previous_on_x, "child " + String(i) + ", 'on.x' greater than previous");
		previous_on_x = mock_this._array.children[i]._.on.x;
		
		t.equal(mock_this._array.children[i]._.on.y, previous_on_y, "child " + String(i) + ", 'on.y' equal to previous");
		previous_on_y = mock_this._array.children[i]._.on.y;
		
		t.ok(check.number(mock_this._array.children[i]._.below.x), "child " + String(i) + " has a numeric 'below.x' property");
		t.ok(check.number(mock_this._array.children[i]._.below.y), "child " + String(i) + " has a numeric 'below.y' property");
		
		t.ok(mock_this._array.children[i]._.below.x > previous_below_x, "child " + String(i) + ", 'below.x' greater than previous");
		previous_below_x = mock_this._array.children[i]._.below.x;
		
		t.equal(mock_this._array.children[i]._.below.y, previous_below_y, "child " + String(i) + ", 'below.y' equal to previous");
		previous_below_y = mock_this._array.children[i]._.below.y;
	}
