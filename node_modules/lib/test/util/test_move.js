...
test('[move.is_moving && move.to] correct, false before move, true while moving, false after and tween completed by changing the source', function (t) {
	...
	//BUG FIX: Some other tweening with move is happening during this attempted move, messing up the test
	//	just delaying the testing for the default duration of other move()s, since no test in this file before this would tween
	//	and some other async tween from some other test is interfering
	setTimeout(function() {
...
test('[move.whats_moving] correct', function (t) {
	//SETUP
	SETUP();
	...
	setTimeout(function() {
		t.ok(move.is_moving(), "should return true during a tween");
		t.deepEqual(move.whats_moving(), test_obj, "should see a reference to the test object being tweened");
	}, 500);
