...
test('[move] incorrect, source, does not have matching properties', function (t) {
	t.plan(2);
	...
	try {
		...
		t.ok(check.instanceStrict(error, TypeError), 'we should get back a TypeError');
		var message = error.toString();
		t.ok(message.match(/is not present on \'source\' to tween/), 'should tell you what is wrong');
	}
});
...
test('[move] incorrect, objects have the (x, y) properties, but wrong type', function (t) {
	...
	try {
		...
		t.ok(check.instanceStrict(error, TypeError), 'we should get back a TypeError');
		var message = error.toString();
		t.ok(message.match(/should be numeric/), 'should tell you what is wrong');
	}
});

test('[move] incorrect, objects have matching properties, but source is non-numeric', function (t) {
	t.plan(2);
	
	try {
		move({source:{x:"0", y:0}, destination:{x:0}, createjs:{}});
	} catch(error) {
		t.ok(check.instanceStrict(error, TypeError), 'we should get back a TypeError');
		var message = error.toString();
		t.ok(message.match(/on \'source\' is not numeric/), 'should tell you what is wrong');
	}
});
...
test('[move] (integration) correct, inegration testing, defaults', function (t) {
	t.plan(6);
...
test('[move] (integration) correct, inegration testing, duration set', function (t) {
	t.plan(6);
	var mock_source = {x:0, y:0}
...
test('[move] (integration) correct, source and desintation the same', function (t) {
	t.plan(4);
	var mock_source = {x:0, y:0};
...
test('[move] (integration) correct, single property tweened', function (t) {
	t.plan(4);
	var mock_source = {x:0, y:0};
	var mock_destination = {y:1};
	...
	setTimeout(function() {
		t.equal(mock_source.x, 0, 'value should be untouched');
		t.equal(mock_source.y, 1, 'value should be tweened to match the destination');
	}, 2000);
});