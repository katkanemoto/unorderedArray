...
function array_UI(options) {
	...
	this._array = this._decorate_method_pop(this._array, this.arrow);
...
array_UI.prototype = {
	...
	//NOTE: the values internal array and the UI will be __out of sync__ until the other values in the UI have been moved to fill the spot left by the removed item
	//	and a prev() has been called on the "tail" arrow (to move it backwards in the UI)
		...
		array._.pop = function(options) {
			...
			if (check.not.undefined(options) && check.not.undefined(options.index)) {
				if (check.not.integer(options.index)) {
					throw new TypeError("Optional argument 'index', should be an integer and in bounds on the array.");
				} else {
					if (options.index >= 0 && options.index < arrow._.index) {
						index = options.index
					} else {
						throw new RangeError("Optional argument 'index', should be an integer and in bounds on the array.");
					}
				}
			}
			...
			var ret = (array._.values.splice(index, 1))[0];
	...
	//NOTE: as it is possible for the values array and the UI representation to be out of sync (e.g. via pop()),
	//	the UI array is ONLY examined in this method and it is assumed that this method is used to synchronize
	//	the UI with the internal values array after a change to the array
	_decorate_method_move_within : function(array, arrow) {
		array._.move_within = function(src, dst, options) {
			...
			array.children[destination].addChild(source_element);	//addChild() side effect is removing from its original position
			
			//if we're tweening, then set the source elements' coordinates to values such that it appears in the same as its original position
			//	then move() to (9, 15)
			if (tween) {
			...
			//return the modified UI array to caller (so they can verify it matches with the internal values array)
			var ret = [];
			for (var i = 0; i < arrow._.index; i++) {
				if(check.not.undefined(array.children[i].children[1])) {
					ret.push(Number(array.children[i].children[1].text));
				} else {
					ret.push(null);
				}
			};
